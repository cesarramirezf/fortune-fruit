function CNextCardPanel(oParentContainer) {
  ...
  this._init = function () {
    ...
    _oButNext = new CTextButton(
      CANVAS_WIDTH / 2,
      CANVAS_HEIGHT / 2 + 300,
      s_oSpriteLibrary.getSprite("but_generic"),
      TEXT_PLAY_NEXT,
      FONT,
      FONT_COLOR,
      60,
  ...
  this.show = function (iType, bWin, bGameOver) {
    ...
    if (bWin) {
      ...
      createjs.Tween.get(_oContainerWin, { loop: -1 })
        .to({ scale: 1.1 }, 850, createjs.Ease.quadOut)
        .to({ scale: 1 }, 650, createjs.Ease.quadIn);
