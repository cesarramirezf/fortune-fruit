function CPaytable(oParentContainer) {
  ...
  this._init = function () {
    ...
    _pStartPosExitLandscape = { x: 600, y: -250 };
  ...
  this._createPortraitPanel = function () {
    ...
    for (var k = 0; k < NUM_SYMBOLS; k++) {
      ...
      var oSymbol = createBitmap(
        s_oSpriteLibrary.getSprite("symbol_" + k + "_" + iFrame)
      );
      oSymbol.x = iXPos;
      oSymbol.y = iYPos;
      ...
      var oText = new CTLText(
        _oContainerPortrait,
        iXPos,
        iYPos + LOGIC_CELL_HEIGHT * iScale + 10,
        LOGIC_CELL_WIDTH * iScale,
        40,
        40,
        "center",
        FONT_COLOR,
  ...
  this.hide = function () {
    ...
    createjs.Tween.get(_oContainerPanel)
      .to({ scale: 0 }, 600, createjs.Ease.backIn)
      .call(function () {
        _oContainer.visible = false;
      });
  };
  ...
  this.refresh = function () {
    if (s_bLandscape) {
      ...
      _oButExit.setPosition(
        _pStartPosExitLandscape.x,
        _pStartPosExitLandscape.y
      );
    } else {
