function CGridCell(iX, iY, iRow, iCol, iType, oParentContainer) {
  ...
  this._init = function (iX, iY, iType) {
    ...
    var oData = {
      ...
      animations: {
        start: START_FRAME_SYMBOL[iType],
        anim: [0, aSpritesSymbols.length - 1, "stop_anim"],
        stop_anim: aSpritesSymbols.length - 1,
        anim_loop: [0, aSpritesSymbols.length - 1],
      },
    };

    var oSpriteSheet = new createjs.SpriteSheet(oData);
    _oSymbolSprite = createSprite(
      oSpriteSheet,
      "start",
      0,
      0,
    ...
    var oData = {
      ...
      // width, height & registration point of each sprite
      frames: {
        width: aSpritesFrame[0].width,
        height: aSpritesFrame[0].height,
        regX: aSpritesFrame[0].width / 2,
        regY: aSpritesFrame[0].height / 2,
      },
      animations: {
        start: 0,
        anim: [0, aSpritesFrame.length - 1, "stop_anim"],
        stop_anim: aSpritesFrame.length - 1,
        anim_loop: [0, aSpritesFrame.length - 1],
      },
    };

    var oSpriteSheet = new createjs.SpriteSheet(oData);

    _oWinFrame = createSprite(
      oSpriteSheet,
      "start",
      aSpritesFrame[0].width / 2,
      aSpritesFrame[0].height / 2,
      aSpritesFrame[0].width,
      aSpritesFrame[0].height
    );
    ...
    _oTextWin = new CTLText(
      _oContainerWin,
      25,
      10,
      oSpriteWin.width - 50,
      oSpriteWin.height - 20,
      oSpriteWin.height - 40,
      "center",
      FONT_COLOR,
      FONT2,
      1.1,
    ...
    var oData = {
      ...
      frames: {
        width: iWidthSprite,
        height: iHeightSprite,
        regX: iWidthSprite / 2,
        regY: iHeightSprite / 2,
      },
      animations: {
        start: 0,
        anim: [0, aSprites.length - 1, "stop_anim"],
        stop_anim: aSprites.length - 1,
      },
    };

    var oSpriteSheet = new createjs.SpriteSheet(oData);

    _oSilver = createSprite(
      oSpriteSheet,
      "start",
      iWidthSprite / 2,
      iHeightSprite / 2,
  ...
  this.playWinAnim = function () {
    createjs.Tween.get(_oContainer, { loop: 2 })
      .to({ rotation: 5 }, 50, createjs.Ease.quadOut)
      .to({ rotation: -5 }, 100, createjs.Ease.quadIn)
      .to({ rotation: 0 }, 50, createjs.Ease.quadIn);
  };
  ...
  this._onEndReveal = function (evt) {
    if (evt.name === "stop_anim") {
      if (_aCbCompleted[ON_SELECT_CELL]) {
        _aCbCompleted[ON_SELECT_CELL].call(
          _aCbOwner[ON_SELECT_CELL],
          _iRow,
          _iCol,
          _iType
        );
