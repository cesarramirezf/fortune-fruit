function CBoard(oParentContainer) {
  ...
  this._prepareSymbolsList = function (iWin, iSymbolIndex) {
    ...
    if (iWin === 0) {
      ...
      //WIN
      var aPattern = this.generateLosingPattern(NUM_SYMBOLS - 3, [
        iSymbolIndex,
      ]);
      aFinalPattern = aPattern.concat([
        iSymbolIndex,
        iSymbolIndex,
        iSymbolIndex,
      ]);
  ...
  this._createGrid = function (aPattern) {
    ...
    _oContainer.regX =
      (LOGIC_CELL_WIDTH * GRID_SIZE + SPACE_BETWEEN_CELLS * (GRID_SIZE - 1)) /
        2 -
      LOGIC_CELL_WIDTH / 2;
    _oContainer.regY =
      (LOGIC_CELL_HEIGHT * GRID_SIZE + SPACE_BETWEEN_CELLS * (GRID_SIZE - 1)) /
        2 -
      LOGIC_CELL_HEIGHT / 2;
  };
  ...
  this._checkIfWin = function () {
    var oCounts = {};
    _aVisibleSymbols.forEach(function (x) {
      oCounts[x] = (oCounts[x] || 0) + 1;
    });
      ...
      setTimeout(
        function () {
          if (_aCbCompleted[ON_END_SCRATCHING]) {
            _aCbCompleted[ON_END_SCRATCHING].call(
              _aCbOwner[ON_END_SCRATCHING],
              true
            );
          }
        },
  ...
  this.enlightSymbolsByType = function (iType) {
    for (var i = 0; i < GRID_SIZE; i++) {
      for (var j = 0; j < GRID_SIZE; j++) {
        if (
          _aGridCells[i][j].isScratched() &&
          _aGridCells[i][j].getType() == iType
        ) {
